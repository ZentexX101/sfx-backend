...
const globalErrorHandler = (err, req, res, next) => {
  ...
  let description =
    err.description || "An unexpected error occurred. Please try again later."; // Use description from AppError
  let errors = {};
  ...
  // 🔹 Handle Multer Errors (file size / count limit)
  if (err.name === "MulterError") {
    statusCode = StatusCodes.BAD_REQUEST;
    message = "File Upload Error";

    if (err.code === "LIMIT_FILE_SIZE") {
      description = "Uploaded file exceeded size limit.";
    } else if (err.code === "LIMIT_FILE_COUNT") {
      description = "You can upload up to 3 images only.";
    } else {
      description = err.message || "Invalid file upload request.";
    }
  }
  ...
  if (err.name === "MongoError") {
    ...
    description =
      "An unknown database error occurred. Please check the logs for more details.";
  }
  ...
  res.status(statusCode).json({
    ...
    error: {
      code: statusCode,
      description:
        err._message || description || "Something went wrong on the server.",
